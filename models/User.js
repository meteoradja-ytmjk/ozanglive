...
class User {
  ...
  static findById(id) {
    return new Promise((resolve, reject) => {
      ...
      db.get('SELECT id, username, avatar_path, user_role, status, live_limit, storage_limit, storage_used, expired_at, created_at FROM users WHERE id = ? LIMIT 1', [id], (err, row) => {
        if (err) {
  ...
  static findByIdWithPassword(id) {
    return new Promise((resolve, reject) => {
      db.get('SELECT id, username, password, avatar_path, user_role, status, live_limit, storage_limit, storage_used FROM users WHERE id = ? LIMIT 1', [id], (err, row) => {
        if (err) {
