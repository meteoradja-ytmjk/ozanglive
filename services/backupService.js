...
function validateComprehensiveBackup(data) {
  ...
  // Check for metadata (optional for old backups)
  if (!data.metadata) {
    console.log('[Validation] No metadata found - assuming legacy backup format');
    // Add default metadata for old backups
    data.metadata = {
      version: '1.0.0',
  ...
  // Also check for old format field names
  const hasLegacyData = data.streams || data.settings || data.configurations;
  
  if (!hasArrayCategory && !hasThumbnailFiles && !hasLegacyData) {
    errors.push('Invalid backup format: no valid data categories found');
    return { valid: false, errors };
  }

  // Migrate legacy field names if needed
  if (data.settings && !data.streams) {
    console.log('[Validation] Migrating legacy "settings" to "streams"');
    data.streams = data.settings;
    delete data.settings;
  }
  if (data.configurations && !data.streams) {
    console.log('[Validation] Migrating legacy "configurations" to "streams"');
    data.streams = data.configurations;
...
async function importThumbnailFilesData(thumbnailData, userId, options = {}) {
  ...
  if (!thumbnailData) {
    console.log('[importThumbnailFiles] No thumbnail data provided');
    return result;
  }
  ...
  try {
    ...
    console.log(`[importThumbnailFiles] Created/verified user thumbnail directory: ${userThumbnailsDir}`);
  } catch (err) {
    console.error('[importThumbnailFiles] Error creating thumbnail directory:', err);
    result.errors.push(`Failed to create thumbnail directory: ${err.message}`);
    return result;
  }
  ...
  if (thumbnailData.template_folder_mapping && Array.isArray(thumbnailData.template_folder_mapping)) {
    ...
    console.log(`[importThumbnailFiles] Loaded ${result.folderMap.size} template folder mappings`);
  }
  
  // Import folder-based thumbnails
  if (thumbnailData.folders && Array.isArray(thumbnailData.folders)) {
    console.log(`[importThumbnailFiles] Importing ${thumbnailData.folders.length} thumbnail folders`);
    ...
    for (const folder of thumbnailData.folders) {
      ...
      try {
        ...
        if (folder.files && Array.isArray(folder.files)) {
          console.log(`[importThumbnailFiles] Processing ${folder.files.length} files in folder ${folder.folder_id}`);
          ...
          for (const file of folder.files) {
            ...
            try {
              ...
              if (!file.data || typeof file.data !== 'string') {
                result.errors.push(`thumbnail ${folder.folder_id}/${file.filename}: Invalid or missing base64 data`);
                result.skipped++;
                continue;
              }
              ...
              if (exists) {
                result.overwritten++;
                console.log(`[importThumbnailFiles] Overwritten thumbnail: ${folder.folder_id}/${file.filename}`);
              } else {
                result.imported++;
                console.log(`[importThumbnailFiles] Imported thumbnail: ${folder.folder_id}/${file.filename}`);
              }
            } catch (fileErr) {
              console.error(`[importThumbnailFiles] Error importing file ${folder.folder_id}/${file.filename}:`, fileErr);
        ...
        console.error(`[importThumbnailFiles] Error processing folder ${folder.folder_id}:`, folderErr);
  ...
  if (thumbnailData.files && Array.isArray(thumbnailData.files)) {
    console.log(`[importThumbnailFiles] Importing ${thumbnailData.files.length} standalone thumbnail files`);
    ...
    for (const file of thumbnailData.files) {
      ...
      try {
        // Validate base64 data
        if (!file.data || typeof file.data !== 'string') {
          result.errors.push(`thumbnail ${file.filename}: Invalid or missing base64 data`);
          result.skipped++;
          continue;
        }
        ...
        if (exists) {
          result.overwritten++;
          console.log(`[importThumbnailFiles] Overwritten standalone thumbnail: ${file.filename}`);
        } else {
          result.imported++;
          console.log(`[importThumbnailFiles] Imported standalone thumbnail: ${file.filename}`);
        }
      } catch (fileErr) {
        console.error(`[importThumbnailFiles] Error importing standalone file ${file.filename}:`, fileErr);
  ...
  console.log(`[importThumbnailFiles] Thumbnail import complete for user ${userId}: ${result.imported} new, ${result.overwritten} overwritten, ${result.skipped} failed, ${result.folderMap.size} template mappings`);
  
  return result;
}

/**
 * Comprehensive import of all user data with progress tracking
 * @param {Object} backupData - Backup data object
 * @param {string} userId - User ID
 * @param {Object} options - Import options (skipDuplicates, overwrite, progressCallback)
...
async function comprehensiveImport(backupData, userId, options = {}) {
  ...
  console.log('[comprehensiveImport] Starting import for user:', userId);
  console.log('[comprehensiveImport] Options:', options);

  // Progress callback helper
  const reportProgress = (step, current, total, message) => {
  ...
  if (!validation.valid) {
    console.error('[comprehensiveImport] Validation failed:', validation.errors);
    return {
      success: false,
  ...
  const steps = [];
  if (backupData.thumbnail_files) steps.push('thumbnail_files');
  if (backupData.youtube_credentials) steps.push('youtube_credentials');
  if (backupData.streams) steps.push('streams');
  if (backupData.broadcast_templates) steps.push('broadcast_templates');
  if (backupData.stream_templates) steps.push('stream_templates');
  if (backupData.recurring_schedules) steps.push('recurring_schedules');
  if (backupData.playlists) steps.push('playlists');
  if (backupData.title_folders) steps.push('title_folders');
  if (backupData.title_suggestions) steps.push('title_suggestions');
  ...
  try {
    ...
    if (backupData.thumbnail_files) {
      currentStep++;
      reportProgress('thumbnail_files', currentStep, totalSteps, 'Importing thumbnail files...');
      console.log('[comprehensiveImport] Importing thumbnail files...');
      try {
        ...
        console.log(`[comprehensiveImport] Thumbnail folder map has ${thumbnailFolderMap.size} entries`);
        
        if (thumbnailResult.errors.length > 0) {
          results.warnings.push(`Thumbnail import had ${thumbnailResult.errors.length} errors`);
        }
      } catch (thumbnailError) {
        console.error('[comprehensiveImport] Error importing thumbnails:', thumbnailError);
        results.results.thumbnail_files = {
          imported: 0,
          overwritten: 0,
          skipped: 0,
        ...
        results.warnings.push(`Thumbnail import failed: ${thumbnailError.message}`);
      }
    }
    ...
    if (backupData.youtube_credentials) {
      currentStep++;
      reportProgress('youtube_credentials', currentStep, totalSteps, 'Importing YouTube credentials...');
      console.log('[comprehensiveImport] Importing YouTube credentials...');
      try {
        results.results.youtube_credentials = await importYouTubeCredentialsData(
          backupData.youtube_credentials, userId, options
        );
      } catch (error) {
        console.error('[comprehensiveImport] Error importing YouTube credentials:', error);
        results.results.youtube_credentials = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.streams) {
      currentStep++;
      reportProgress('streams', currentStep, totalSteps, 'Importing streams...');
      console.log('[comprehensiveImport] Importing streams...');
      try {
        results.results.streams = await importStreams({ streams: backupData.streams }, userId, options);
      } catch (error) {
        console.error('[comprehensiveImport] Error importing streams:', error);
        results.results.streams = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.broadcast_templates) {
      currentStep++;
      reportProgress('broadcast_templates', currentStep, totalSteps, 'Importing broadcast templates...');
      console.log('[comprehensiveImport] Importing broadcast templates...');
      try {
        ...
        console.error('[comprehensiveImport] Error importing broadcast templates:', error);
        results.results.broadcast_templates = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.stream_templates) {
      currentStep++;
      reportProgress('stream_templates', currentStep, totalSteps, 'Importing stream templates...');
      console.log('[comprehensiveImport] Importing stream templates...');
      try {
        results.results.stream_templates = await importStreamTemplatesData(
          backupData.stream_templates, userId, options
        );
      } catch (error) {
        console.error('[comprehensiveImport] Error importing stream templates:', error);
        results.results.stream_templates = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.recurring_schedules) {
      currentStep++;
      reportProgress('recurring_schedules', currentStep, totalSteps, 'Importing recurring schedules...');
      console.log('[comprehensiveImport] Importing recurring schedules...');
      try {
        results.results.recurring_schedules = await importRecurringSchedulesData(
          backupData.recurring_schedules, userId, options
        );
      } catch (error) {
        console.error('[comprehensiveImport] Error importing recurring schedules:', error);
        results.results.recurring_schedules = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.playlists) {
      currentStep++;
      reportProgress('playlists', currentStep, totalSteps, 'Importing playlists...');
      console.log('[comprehensiveImport] Importing playlists...');
      try {
        results.results.playlists = await importPlaylistsData(
          backupData.playlists, userId, options
        );
      } catch (error) {
        console.error('[comprehensiveImport] Error importing playlists:', error);
        results.results.playlists = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.title_folders) {
      currentStep++;
      reportProgress('title_folders', currentStep, totalSteps, 'Importing title folders...');
      console.log('[comprehensiveImport] Importing title folders...');
      try {
        ...
        console.error('[comprehensiveImport] Error importing title folders:', error);
        results.results.title_folders = { imported: 0, skipped: 0, errors: [error.message] };
    ...
    if (backupData.title_suggestions) {
      currentStep++;
      reportProgress('title_suggestions', currentStep, totalSteps, 'Importing title suggestions...');
      console.log('[comprehensiveImport] Importing title suggestions...');
      try {
        results.results.title_suggestions = await importTitleSuggestionsData(
          backupData.title_suggestions, userId, folderMap, options
        );
      } catch (error) {
        console.error('[comprehensiveImport] Error importing title suggestions:', error);
        results.results.title_suggestions = { imported: 0, skipped: 0, errors: [error.message] };
        results.warnings.push(`Title suggestions import failed: ${error.message}`);
      }
    }

    // Report completion
    reportProgress('complete', totalSteps, totalSteps, 'Import completed successfully');
    console.log('[comprehensiveImport] Import completed successfully');
    console.log('[comprehensiveImport] Results:', JSON.stringify(results.results, null, 2));
    
  } catch (error) {
    console.error('[comprehensiveImport] Unexpected error during import:', error);
    console.error('[comprehensiveImport] Stack trace:', error.stack);
    return {
      success: false,
