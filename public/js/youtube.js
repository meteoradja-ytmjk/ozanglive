    ...
    return `
      <div class="broadcast-list-item broadcast-row hover:bg-dark-700/30 transition-colors" 
        ...
        <div class="md:hidden px-3 py-2.5 flex items-center justify-between gap-2 bg-dark-800/40 hover:bg-dark-750/50 border-b border-gray-700/50 last:border-b-0 transition-colors">
          <!-- Left: Checkbox + Info Column (Title on row 1, Privacy & Date & StreamKey on row 2) -->
          <div class="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div class="flex-1 min-w-0">
              <!-- Row 1: Badge #Index + Title (Full width available on single line) -->
              ...
              <div class="flex items-center gap-1.5 text-[11px] text-gray-400 mt-1 min-w-0 overflow-hidden">
                <span class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide shrink-0 ${privacyClass}">
                ...
                ${broadcast.streamKey ? `
                  <span class="text-gray-600 shrink-0">•</span>
                  <button type="button" onclick="copyStreamKey('${escapeJsString(broadcast.streamKey || '')}', ${index + 1})"
                    class="inline-flex items-center gap-1 font-mono text-[11px] text-gray-400 hover:text-primary active:text-primary/80 transition-colors truncate max-w-[110px] cursor-pointer"
                    title="Klik untuk salin: ${escapeHtml(broadcast.streamKey || '')}">
                    <i class="ti ti-key text-xs text-primary/80 shrink-0 font-loaded"></i>
                    <span class="truncate">${broadcast.streamKey.substring(0, 8)}...</span>
                  </button>
                ` : '<span class="text-gray-600 shrink-0">•</span><span class="text-gray-500 font-mono text-[10px]">-</span>'}
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1 shrink-0">
            ...
            <button type="button" data-action="delete" onclick="deleteBroadcast('${broadcast.id}', null, ${broadcast.accountId || 'null'}, this)"
              class="w-7 h-7 rounded-lg bg-red-500/15 hover:bg-red-500/25 active:bg-red-500/40 text-red-400 border border-red-500/30 flex items-center justify-center transition-all duration-150 active:scale-90 touch-manipulation cursor-pointer shadow-sm select-none" title="Hapus">
  ...
  templates.forEach((template, index) => {
    ...
    div.innerHTML = `
      ...
      <div class="flex items-center justify-between gap-2 sm:gap-3 w-full">
        ...
        <!-- Sisi Kanan: Action Buttons (Square Hapus Jadwal, Rounded Rect Rebroadcast, Square Trash) -->
